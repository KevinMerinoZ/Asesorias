...
async function validarCreacionMaterialCompartido(idForm, event) {
    // Obtener el formulario usando el ID proporcionado.
    const formulario = document.getElementById(idForm);
    ...
    let flag = validarTamanoArc(idForm, event);
    ...
    if (flag) {
        try {
            ...
            if(respuesta == 0){
                alert("El material no coincide con el tipo de material seleccionado");
            }else if(respuesta == 1){
                const formTemp = document.createElement('form');
                formTemp.method = "POST";
                formTemp.action = "index.php";

                const inputHiddenOpc = document.createElement('input');
                inputHiddenOpc.name = "opc";
                inputHiddenOpc.value = "vistaGestionMaterialCompartido";

                const inputHiddenIdCita = document.createElement('input');
                inputHiddenIdCita.name = 'idCita';
                inputHiddenIdCita.value = formulario.querySelector('[name="idCita"]').value;

                formTemp.appendChild(inputHiddenIdCita);
                formTemp.appendChild(inputHiddenOpc);
