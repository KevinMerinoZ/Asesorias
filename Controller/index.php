...
class Controlador{
    ...
    public function respaldo() {
        $rutaDestino = 'Model/respaldos';
        $mensaje = $this->modelo->realizarRespaldo($rutaDestino);
        $this->vistaInicioDirectivo();
    }

    public function restaurar() {
        $archivos = glob('Model/respaldos/*.sql');
        if (!empty($archivos)) {
            $archivoMasReciente = end($archivos);
            $mensaje = $this->modelo->restaurarRespaldo($archivoMasReciente);
        } else {
            $mensaje = "No hay archivos de respaldo disponibles.";
        }
        $this->vistaInicioDirectivo();
    }  
    ...
    function vistaGestionMaterialCompartido($result = null){
        ...
        if($_SESSION['tipoUsuario'] == 'Profesor'){
            ...
            $tipoMaterial = $this->modelo->consultarTodoTipoMaterialPorCat($_POST['idProfesor']);
            $vistaAnt = "vistaGestionCitas";
            require_once("View/Users/Alumno/Gestiones/MCAlumno.php");
            $_SESSION['vistaActual'] = "vistaGestionMaterialCompartido";
        }
    ...
    function crearDirectivo(){
        ...
        $result = $this->modelo->crearDirectivo($matricula, $contrasenia, $nombre, $apellidoP, $apellidoM, $genero, $departamento, $noProfesores);
    ...
    function actualizarDirectivo(){
        ...
        $departamento = $_POST['departamento'];
        $noProfesores = $_POST['noProfesores'];
        $result = $this->modelo->actualizarDirectivo($matricula, $nombre, $apellidoP, $apellidoM, $genero, $departamento, $noProfesores);

        $this->vistaGestionDirectivos();
    }

    /*************** Gestion de Asignaturas ***************/
    function consultarUnaAsignatura(){
        $nombre = $_POST['nombre'];
        $result = $this->modelo->consultarUnaAsignatura($nombre);
        
        $this->vistaGestionAsignaturas($result);
    }
